...
module.exports = (target, cb) => {
    return gitRepos(target, { ignore }, (err, repoPath) => {
...
const ignoreDirs = ['node_modules', 'coverage', '.nyc_output', 'lib-cov', 'dist', 'build', 'logs', 'android', 'ios'];

function ignore(dir) {
  return ignoreDirs.includes(path.basename(dir));
}
